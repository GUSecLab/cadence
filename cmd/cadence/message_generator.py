...
query = """
    ...
    FROM encounters
    WHERE dataset_name = %s AND time >= %s AND time <= %s and distance= %s
    ORDER BY time
    
"""
params = (dataset_name,start_time,end_time,distance)# float(sys.argv[6]))  # Replace `startTime` with the appropriate value
